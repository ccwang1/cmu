...
class Bullet(object):
    def __init__(self,x1,y1,x2,y2,bullettype='normal',damage=1):
        self.towerx1=x1
        self.towery1=y1
        self.x=x1
        self.y=y1
...
class game(eventBasedAnimation.Animation):
    ...
    def mouseGameStart(self,event):
        ...
        if 0<event.x<40 and 600<event.y<640:
            if self.money>=25:
                self.placeFreeze=False
                self.build=True
                self.placeBasic=True    #test
                self.showinfo=False
        if 40<event.x<80 and 600<event.y<640:
            if self.money>=40:
                self.placeBasic=False
                self.build=True
                self.placeFreeze=True
                self.showinfo=False
        if 0<=floorx<600 and 0<=floory<600:
            ...
            #         pass
            elif type(spot)==Basictower or type(spot)==Freezetower: 
            #for displaying info of tower
                if type(spot)==Basictower:
                    ...
                    self.infoindex=self.towers.index(Freezetower(floorx+20,
                        floory+20))
                self.showinfo=True
            if not (type(spot)==Basictower or type(spot)==Freezetower):
                self.showinfo=False
            self.build=False
    ...
    def freezeInfo(self,canvas):
        ...
        canvas.create_rectangle(center-margin,325,center+margin,500,
        fill='light green')  #inf
        canvas.create_text(center,340,text='The Freeze Tower has',font=('Courier',11))
        canvas.create_text(center,355,text='a crippling attack',font=('Courier',11))
        canvas.create_text(center,370,text='which permanently ',font=('Courier',11))
        canvas.create_text(center,385,text='slows any monster it',font=('Courier',11))
        canvas.create_text(center,400,text='hits by 50%. Though ',font=('Courier',11))
        canvas.create_text(center,415,text='it may be expensive,',font=('Courier',11))
        canvas.create_text(center,430,text='this tactical tower ',font=('Courier',11))
        canvas.create_text(center,445,text='can strengthen your',font=('Courier',11))
        canvas.create_text(center,460,text='defense significantly.',font=('Courier',11))
    ...
    def showInfo(self,canvas):
        ...
        # samplebasic=Basictower(20,self.height-20)   #basictower
        ...
        # samplefreeze=Freezetower(60,self.height-20) #freezetower
                ...
                if closest<tower.range:
                    if closemx != None and closemy!=None:
                        self.bullets.append(Bullet(tower.x,tower.y,closemx,
                            closemy,tower.type,tower.damage))
...
game(width=800, height=640,windowTitle='Snake Tower Defense').run()